 ...
 * left-aligned text cols 1–5 + imagery cols 7–12 on desktop, stacked with the
 * image last on mobile. Exactly two CTAs; missing imagery becomes a seal
 * composition. Generous negative space is the composition.
 */
export function HomeHero({ hero, identity }: HomeHeroProps) {
  const fallbackPanel = (
    <div className="flex h-full w-full flex-col items-center justify-center gap-4 p-8 text-center">
      ...
      <p className="type-body text-primary">{identity.tagline}</p>
    </div>
  );

  return (
    <section className="container-page">
      <div className="flex flex-col gap-10 py-16 lg:grid lg:min-h-[calc(100dvh-4rem)] lg:grid-cols-12 lg:items-center lg:gap-x-12 lg:py-24">
        <div className="relative lg:col-span-5 lg:pl-8">
          {hero.eyebrow ? (
            <p className="type-label mb-4 text-text-muted">{hero.eyebrow}</p>
          ) : null}
